...
# CSV_PATH_TRAIN = BASE_DIR / "Scripts" / 'DDSM'/ 'samples'/ "0_e35e53fd-5312-406e-ad9f-4c11e725e53f.dcm"
CSV_PATH_TRAIN = BASE_DIR / "Scripts" / 'data'/ 'DDSM'/ 'samples'
...
print("Exists:", CSV_PATH_TRAIN.exists())
print("Is directory:", CSV_PATH_TRAIN.is_dir())

# 3) Try to find image files
exts = {".png", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff", ".dcm"}
files = [p for p in CSV_PATH_TRAIN.iterdir() if p.is_file() and p.suffix.lower() in exts]


# print(files)
print("Image-like files found:", len(files))
print("First 5 files:")
for p in sorted(files, key=lambda p: p.name)[:9]:
    print(" -", p.name)
