...
# This version: custom dataset, DataLoader, and training pass 

# To build paths inside a project like this: BASE_DIR / "subdir".
BASE_DIR = Path(__file__).resolve().parent.parent
DATA_DIR = BASE_DIR / "Scripts" / "data"
CSV_PATH = BASE_DIR / "Scripts" / "miniddsm_image_labels.csv"
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (real_images, real_targets) in enumerate(train_loader):
            ...
            if batch_idx % 1 == 0 and batch_idx > 0:
                print(f"Epoch {epoch + 1}/{epochs} | Batch {batch_idx}, RunningLoss: {running_loss / 100:.4f}")
                running_loss = 0.0
...
def main():
    device = get_device()
    data_loader,label_map = prepare_data()
    print(f"Data loader and label map prepared. Number of batches: {len(data_loader)}")
    print(f"Label map: {label_map}")

    model = MinimalCNN().to(device)
    train_model(model, data_loader, device, epochs=2)
